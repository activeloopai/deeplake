...
def _strip_sql_comments(sql_text):
    """Remove SQL comments to simplify statement-type detection."""
...
def _ddl_guardrails(sql_text):
    """Return (ok, reason) for conservative DDL-only execution checks."""
    cleaned = _strip_sql_comments(sql_text)
    if not cleaned:
        return False, "empty SQL statement"

    # Allow one statement by default; multi-statement requires --force.
    ...
    if not m:
        return False, "unable to determine SQL statement type"
    ...
    if keyword not in allowed:
        return False, f"statement '{keyword}' is not in the default DDL allowlist"
...
def cmd_run_ddl(args):
    ...
    if not args.force:
        ok, reason = _ddl_guardrails(ddl)
        if not ok:
            print(f"  ERROR: blocked by run-ddl safety guard: {reason}")
...
def cmd_rotate_creds(args):
    ...
    for inst in instances:
        ...
        result = subprocess.run(
            ["docker", "exec", "-i", container, "tee", "/dev/shm/deeplake-creds.conf"],
            input=conf_content + "\n",
            capture_output=True,
            text=True,
        )
        ...
        chmod_result = subprocess.run(
            ["docker", "exec", container, "chmod", "600", "/dev/shm/deeplake-creds.conf"],
            capture_output=True,
            text=True,
        )
        if chmod_result.returncode != 0:
            print(f"  {container}: FAILED chmod tmpfs file - {chmod_result.stderr.strip()}")
            failures.append(container)
            continue
...
def main():
    ...
    p_ddl.add_argument(
        "--force",
        action="store_true",
        help="Bypass DDL safety guard (allows non-DDL/multi-statement SQL)",
    )
