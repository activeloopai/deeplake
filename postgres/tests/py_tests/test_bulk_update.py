"""
Test bulk UPDATE operations with deeplake storage.

Ported from: postgres/tests/sql/bulk_update.sql
"""
import pytest
import asyncpg
...
@pytest.mark.asyncio
async def test_bulk_update(db_conn: asyncpg.Connection):
    """
    ...
    - Updates with string concatenation
    - Updates with CASE expressions
    """
    ...
    try:
        ...
        # Test update with string concatenation
        await db_conn.execute("""
            UPDATE people SET name = 'updated_' || name WHERE age <= 17
        """)
        ...
        # Verify final state
        results = await db_conn.fetch("SELECT name, age FROM people ORDER BY age")
        assert len(results) == 4, f"Expected 4 rows, got {len(results)}"

        print("✓ Test passed: Bulk UPDATE operations work correctly")
