...
def pytest_addoption(parser):
    ...
    parser.addoption(
        CACHE_OPT,
        action="store_true",
        help=f"Tests using the `storage` fixture may run with combinations of all enabled providers in cache chains. "
        "For example, if the option `{S3_OPT}` is not provided, all cache chains that use `S3Provider`"
        "  are skipped.",
    )
    parser.addoption(
        CACHE_ONLY_OPT,
