...
class casted_column_view : public chained_column_view
{
public:
    casted_column_view(heimdall::column_view_ptr s, nd::dtype dtype)
        : chained_column_view()
        , source_(s)
        , dtype_(dtype)
    {
    ...
    {
        auto ndt = source_->type().data_type();
        auto new_type = nd::type::cast(ndt, dtype_);
        return deeplake_core::type::generic(new_type);
    }
