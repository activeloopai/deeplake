...
#include "spatial_column.hpp"
#include "sequence_column.hpp"
 ...
 * The group dataset will contain columns with the same column name, dtype, and htype of samples
 ...
 * @note Currently sequence and spatial columns are supported.
 * @param source The source dataset.
 * @param lengths The lengths of the groups.
 * @return dataset_view_ptr The sequence dataset.
 */
template <typename T>
requires(std::is_same_v<T, spatial_column> || std::is_same_v<T, sequence_column>)
heimdall::dataset_view_ptr create_group_dataset(heimdall::dataset_view_ptr source, const std::vector<int64_t>& lengths);

/**
 * @brief Given the source group dataset, creates a dataset with the same columns, but the groups are unrolled.
...
/**
 * @brief Given the source group dataset, creates a dataset with the same columns, but the sequences are split with the given ranges.
 * @param source The source group dataset.
...
/**
 * @brief Given the source group dataset, creates a dataset with the same columns, but the sequences are split with
 * the given lengths.
 * @param source The source group dataset.
