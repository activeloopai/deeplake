...
size_t nifti_assign_buffer(znzFile fp, void** data, size_t ntot, nifti_image *nim)
{
  size_t ii;
...
{
   if( nim != NULL && nim->data != NULL ){
     if ( nifti_image_is_data_owner( nim ) ) {
          free(nim->data);
      }
      nim->data = NULL;
...
nifti_image * nifti_copy_nim_info(const nifti_image * src)
{
  ...
  // copy nifti_image up to impl reserved bytes
  memcpy(dest, src, nifti_image_impl_reserved_bytes_offset());
