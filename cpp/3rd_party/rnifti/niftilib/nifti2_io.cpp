...
static void nifti_image_mem_load_prep( znzFile fp, nifti_image *nim, const uint8_t *data , const size_t size, int gz, const size_t estimated_size);
...
void * nifti2_read_header( const char *hname, int *nver, int check )
{
   ...
   if( ni_ver == 0 || ni_ver == 1 ) {
      ...
      if ( check && ! nifti_hdr1_looks_good(static_cast<nifti_1_header *>(hresult)) ){
         LNI_FERR(fname,"nifti_1_header looks bad for file", hname);
         return hresult;
      }
   } else if ( ni_ver == 2 ) {
      ...
      if ( check && ! nifti_hdr2_looks_good(static_cast<nifti_2_header *>(hresult)) ){
         LNI_FERR(fname,"nifti_2_header looks bad for file", hname);
         return hresult;
      }
   } else {
...
nifti_image *nifti2_image_mem_read( const uint8_t *data , const size_t size, int gz, const size_t estimated_size )
{
   ...
   if( gz ) filesize = -1;  /* unknown */
   else     filesize = size;

   /**- open file, separate reading of header, extensions and data */
   fp = znzmemopen(NULL, data, size, "rb", gz, estimated_size);
   if( znz_isnull(fp) ){
      // if( g_opts.debug > 0 ) LNI_FERR(fname,"failed to open header file",hfile);
      // free(hfile);
      return NULL;
   ...
   if( rv < 0 ){
      ...
      nim = nifti2_mem_read_ascii_image( fp, filesize, gz );
      znzclose(fp);
      // free(hfile);
      return nim;
   }
   ...
   if( ni_ver == 0 || ni_ver == 1 ) {
      nim = nifti_convert_n1hdr2nim(n1hdr , gz ? "_.gz" : "_.nii");
      onefile = NIFTI_ONEFILE(n1hdr);
   } else if ( ni_ver == 2 ) {
      /* fill nifti-2 header and convert */
      // if( g_opts.debug > 2 )
      ...
      nim = nifti_convert_n2hdr2nim(n2hdr , gz ? "_.gz" : "_.nii");
      onefile = NIFTI_ONEFILE(n2hdr);
   } else {
      // if( g_opts.debug > 0 )
      //    Rc_fprintf_stderr("** %s: bad nifti im header version %d\n",fname,ni_ver);
   ...
   /**- read the data if desired, then bug out */
   // if( read_data ){
      if( nifti2_image_mem_load( fp, nim, data, size, gz, estimated_size ) < 0 ) {
         znzclose(fp);
         nifti_image_free(nim);          /* take ball, go home. */
         return NULL;
      }
...
int nifti2_image_mem_load(znzFile fp, nifti_image* nim, const uint8_t *data , const size_t size, int gz, const size_t estimated_size)
{
   ...
   /**- open the file and position the FILE pointer */
   nifti_image_mem_load_prep( fp, nim, data, size, gz, estimated_size );
   ...
   int should_assign_data = 0;
#if HAVE_ZLIB
   if (fp->withz) {
       if (fp->zmemptr != NULL) {
           should_assign_data = 1;
       }
   }
#endif
   if (should_assign_data) {
      /**- now that everything is set up, assign the data */
      ii = nifti_assign_buffer(fp,&nim->data,ntot,nim);
   } else {
      /**- if the data pointer is not yet set, get memory space for the image */
      if (nim->data == NULL) {
         nim->data = calloc(1, ntot); /* create image memory */
   ...
   if( ii < ntot ) {
      ...
      nim->data = NULL;
      nim->decompressed_memory_buffer = NULL;
      return -1; /* errors were printed in nifti_read_buffer() */
   }
   ...
   if (fp->withz) {
      if (fp->zmemptr != NULL) {
         // retain the use of decompressed buffer.
         // so when we remove the fp we still have the access to the data.
         nim->decompressed_memory_buffer = fp->zmemptr->buffer;
      }
   }
#endif
   return 0;
...
int64_t nifti2_assign_buffer(znzFile fp, void** data, int64_t ntot, nifti_image *nim)
{
  int64_t ii;
  ...
  if (!fp->withz) {
   Rc_fprintf_stderr("** ERROR: nifti_assign_buffer: should be called only for compressed data\n");
   return -1;
  }

  #if HAVE_ZLIB
   if (fp->zmemptr == NULL) {
      Rc_fprintf_stderr("** ERROR: nifti_assign_buffer: should be called only for zmemptr type. e.g. memory buffer\n");
      return -1;
   }
  #endif
   ii = znzassign(data, 1, ntot, fp); /* data input */

  void *dataptr = *data;
  if (dataptr == NULL) {
       Rc_fprintf_stderr("buffer assing failed\n");
       return -1;
  }
...
{
    return (size_t)&(((nifti_image*)(NULL))->decompressed_memory_buffer);
}

int nifti2_image_is_data_owner(const nifti_image* nim)
{
   if( nim != NULL && nim->data != NULL ) {
      return nim->decompressed_memory_buffer == nullptr;
   }
   return 0;
}
...
{
   if( nim != NULL && nim->data != NULL ) {
      if ( nifti2_image_is_data_owner(nim) ) {
         free(nim->data);
      }
      nim->data = NULL;
      nim->decompressed_memory_buffer = nullptr;
   }
}
...
nifti_image * nifti2_copy_nim_info(const nifti_image * src)
{
  ...
  memcpy(dest, src, nifti2_image_impl_reserved_bytes_offset());
  ...
  dest->data = NULL;
  // retain decompressed_memory_buffer if any, to keep alive.
  dest->decompressed_memory_buffer = src->decompressed_memory_buffer;
