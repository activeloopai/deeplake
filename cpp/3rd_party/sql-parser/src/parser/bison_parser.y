...
%union {
  ...
  hsql::RowLockWaitPolicy lock_wait_policy_t;
     ...
     *********************************/
    // clang-format off
    %destructor { } <fval> <ival> <bval> <join_type> <order_type> <datetime_field> <column_type_t> <column_constraint_t> <import_type_t> <column_constraint_vec> <lock_mode_t> <lock_wait_policy_t> <across_type>
    ...
    %token DELTA FLOAT SPLIT UNGROUP GROUP INDEX INNER LIMIT LOCAL MERGE MINUS ORDER SAMPLE REPLACE PERCENT
    %token OUTER RIGHT TABLE UNION USING WHERE EXPAND OVERLAP CALL CASE CHAR COPY DATE DATETIME
    %token DESC DROP ELSE FILE FROM FULL HASH HINT INTO JOIN
    %token LEFT LIKE LOAD LONG NULL PLAN SHOW TEXT_INTERNAL_TQL THEN TIME
    ...
    %type <bval>                   opt_not_exists opt_exists opt_all
    %type <distinct_description_t> opt_distinct
    ...
    %type <expr>                   slice_literal slice_literal_0_0_0 slice_literal_0_0_1 slice_literal_0_1_0 slice_literal_0_1_1 slice_literal_1_0_0 slice_literal_1_0_1 slice_literal_1_1_0 slice_literal_1_1_1
    %type <expr>                   comp_expr join_condition opt_having case_expr case_list in_expr hint
    %type <across_type>            opt_across;
    %type <whereClause>            opt_where;
    %type <expansion>              opt_expand;
    %type <expr>                   opt_expand_name opt_expand_overlap;
    %type <expr>                   array_expr array_index string_array_index fancy_array_index dynamic_array_index dynamic_array_index_operand null_literal
    %type <expr_vec>               fancy_array_index_list;
    %type <limit>                  opt_limit opt_top
    %type <sample_limit>           opt_sample_limit
    %type <sample>                 sample_desc opt_sample
    %type <order>                  order_desc
    %type <order_type>             opt_order_type
    %type <datetime_field>         datetime_field duration_field
...
| TEXT_INTERNAL_TQL { $$ = ColumnType{DataType::TEXT}; }
...
| opt_with_clause select_with_paren set_operator select_statement opt_order opt_limit opt_sample {
  $$ = $2;
...
select_no_paren : select_clause opt_order opt_limit opt_sample opt_locking_clause {
  $$ = $1;
  $$->order = $2;
...
| select_clause set_operator select_within_set_operation opt_order opt_limit opt_sample opt_locking_clause {
  $$ = $1;
  if ($$->setOperations == nullptr) {
    $$->setOperations = new std::vector<SetOperation*>();
  }
...
select_clause : SELECT opt_top opt_distinct select_list opt_from_clause opt_where opt_expand opt_group opt_ungroup {
  $$ = new SelectStatement();
  $$->limit = $2;
  $$->distinct = $3;
  $$->selectList = $4;
...
opt_where : WHERE expr { $$ = new hsql::WhereClause($2); }
| /* empty */ { $$ = nullptr; };

opt_expand : EXPAND BY INTVAL INTVAL opt_expand_overlap opt_expand_name { $$ = new hsql::Expansion($3, $4, $6, $5); }
| EXPAND BY INTVAL opt_expand_overlap opt_expand_name { $$ = new hsql::Expansion($3, $3, $5, $4); }
| /* empty */ { $$ = nullptr; };

opt_expand_name : AS IDENTIFIER { $$ = hsql::Expr::makeLiteral($2); }
| /* empty */ { $$ = nullptr; };

opt_expand_overlap : OVERLAP bool_literal { $$ = $2; }
| OVERLAP { $$ = Expr::makeLiteral(true); }
| /* empty */ { $$ = Expr::makeLiteral(true); };

opt_across :  ACROSS TIME { $$ = AcrossType::Time; }
            | ACROSS SPACE { $$ = AcrossType::Space; }
            | /* empty */ { $$ = AcrossType::Time; };


opt_group : GROUP BY expr_list opt_having opt_across {
...
opt_ungroup : UNGROUP BY expr {
  $$ = new UnGroupByDescription();
  $$->expr = $3;
} | UNGROUP BY SPLIT {
  $$ = new UnGroupByDescription();
  $$->split = true;
...
opt_sample : sample_desc { $$ = $1; }
| { $$ = nullptr; }

sample_desc : SAMPLE BY expr REPLACE opt_sample_limit { $$ = new SampleDescription($3, $5, true); }
| SAMPLE BY expr opt_sample_limit { $$ = new SampleDescription($3, $4, true); }
| SAMPLE BY expr REPLACE FALSE opt_sample_limit { $$ = new SampleDescription($3, $6, false); }
| SAMPLE BY expr REPLACE TRUE opt_sample_limit { $$ = new SampleDescription($3, $6, true); }
...
opt_sample_limit : LIMIT expr { $$ = new SampleLimitDescription($2, false); }
| LIMIT expr PERCENT { $$ = new SampleLimitDescription($2, true); }
| { $$ = nullptr; }
...
expr_pair_list : expr_alias ':' expr_alias {
  $$ = new std::map<Expr*, Expr*>();
  $$->emplace($1, $3);
}
| expr_pair_list ',' expr_alias ':' expr_alias {
  $1->emplace($3, $5);
...
| IDENTIFIER '(' opt_distinct expr_pair_list ')' { $$ = Expr::makeFunctionRef($1, $4, $3); }
| ALL '(' opt_distinct expr_list ')' { $$ = Expr::makeFunctionRef(strdup("ALL"), $4, $3); }

extract_expr : EXTRACT '(' datetime_field FROM expr ')' { $$ = Expr::makeExtract($3, $5); };

cast_expr : CAST '(' expr AS column_type ')' { $$ = Expr::makeCast($3, $5); };

datetime_field : SECONDS { $$ = kDatetimeSecond; }
...
duration_field : datetime_field;

array_expr : ARRAY '[' expr_list ']' { $$ = Expr::makeArray($3); };

array_index : dynamic_array_index | string_array_index | fancy_array_index;

string_array_index : operand '[' string_literal ']' { $$ = Expr::makeArrayIndex($1, $3->name); $3->name = nullptr; delete $3; };
fancy_array_index : operand '[' fancy_array_index_list ']' { $$ = Expr::makeArrayIndex($1, $3); };

dynamic_array_index_operand : '(' expr ')' { $$ = $2; } | scalar_expr | unary_expr | binary_expr | case_expr | function_expr | extract_expr | cast_expr | array_expr;
dynamic_array_index : operand '[' dynamic_array_index_operand ']' { $$ = Expr::makeArrayIndex($1, $3); };

fancy_array_index_list : slice_literal {
  $$ = new std::vector<Expr*>();
  $$->push_back($1);
} | fancy_array_index_list ',' slice_literal {
  $1->push_back($3);
...
slice_literal : slice_literal_0_0_0 | slice_literal_0_0_1 | slice_literal_0_1_0 | slice_literal_0_1_1 | slice_literal_1_0_0 | slice_literal_1_0_1 | slice_literal_1_1_0 | slice_literal_1_1_1

slice_literal_0_0_0 : ':' { $$ = Expr::makeSlice(nullptr, nullptr, nullptr); } | ':' ':' { $$ = Expr::makeSlice(nullptr, nullptr, nullptr); };
slice_literal_0_0_1 : ':' ':' int_literal { $$ = Expr::makeSlice(nullptr, nullptr, $3); };
slice_literal_0_1_0 : ':' int_literal { $$ = Expr::makeSlice(nullptr, $2, nullptr); } | ':' int_literal ':' { $$ = Expr::makeSlice(nullptr, $2, nullptr); };
slice_literal_0_1_1 : ':' int_literal ':' int_literal { $$ = Expr::makeSlice(nullptr, $2, $4); };
slice_literal_1_0_0 : int_literal | int_literal ':' | int_literal ':' ':' { $$ = Expr::makeSlice($1, nullptr, nullptr); };
slice_literal_1_0_1 : int_literal ':' ':' int_literal { $$ = Expr::makeSlice($1, nullptr, $4); };
slice_literal_1_1_0 : int_literal ':' int_literal { $$ = Expr::makeSlice($1, $3, nullptr); } | int_literal ':' int_literal ':' { $$ = Expr::makeSlice($1, $3, nullptr); };
slice_literal_1_1_1 : int_literal ':' int_literal ':' int_literal { $$ = Expr::makeSlice($1, $3, $5); };
