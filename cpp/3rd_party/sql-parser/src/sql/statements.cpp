...
SampleDescription::SampleDescription(Expr* expr, SampleLimitDescription* limit, bool repeats)
  : expr(expr), limit(limit), repeats(repeats)
{}
...
SampleLimitDescription::SampleLimitDescription(Expr* limit, bool percent) : limit(limit), percent(percent) {}

SampleLimitDescription::~SampleLimitDescription() {
  delete limit;
}
...
UnGroupByDescription::UnGroupByDescription() : expr(nullptr) {}

UnGroupByDescription::~UnGroupByDescription() {
  if (expr != nullptr) {
    delete expr;
  }
}
