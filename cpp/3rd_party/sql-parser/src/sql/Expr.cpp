...
Expr* Expr::makeFunctionRef(char* func_name, std::map<Expr*, Expr*>* exprDict, bool distinct)
{
    Expr* e = new Expr(kExprFunctionRef);
    e->name = func_name;
    e->exprDict = exprDict;
...
Expr* Expr::makeArrayIndex(Expr* expr, char* index)
{
    Expr* e = new Expr(kExprArrayStringIndex);
    e->expr = expr;
    e->name = index;
    return e;
}

Expr* Expr::makeArrayIndex(Expr* expr, std::vector<Expr*>* fancyIndices)
{
    Expr* e = new Expr(kExprArrayIndex);
    e->expr = expr;
    e->exprList = fancyIndices;
    return e;
}

Expr* Expr::makeArrayIndex(Expr* expr, Expr* index)
{
    Expr* e = new Expr(kExprArrayDynamicIndex);
    e->expr = expr;
    e->expr2 = index;
    return e;
}
...
Expr* Expr::makeSlice(Expr* start, Expr* end, Expr* step)
{
    Expr* e = new Expr(kExprSlice);
    e->exprList = new std::vector<Expr*>{start, end, step};
    return e;
}

Expr* Expr::makeDynamicSlice(Expr* expr)
{
    Expr* e = new Expr(kExprSlice);
    e->expr = expr;
    return e;
...
bool Expr::isLiteral() const
{
    return isType(kExprLiteralInt) || isType(kExprLiteralFloat) || isType(kExprLiteralString) ||
           isType(kExprParameter) || isType(kExprLiteralNull) || isType(kExprLiteralDate) ||
