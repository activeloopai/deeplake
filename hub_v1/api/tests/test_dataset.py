...
import hub_v1.api.dataset as dataset
from hub_v1.cli.auth import login_fn
from hub_v1.exceptions import DirectoryNotEmptyException, ClassLabelValueError
import numpy as np
import pytest
from hub_v1 import transform
from hub_v1.api.dataset_utils import slice_extract_info, slice_split, check_class_label
from hub_v1.cli.auth import login_fn
...
from hub_v1.schema import BBox, ClassLabel, Image, SchemaDict, Sequence, Tensor, Text
from hub_v1.utils import (
    azure_creds_exist,
    gcp_creds_exist,
    hub_creds_exist,
    minio_creds_exist,
...
def test_dataset_change_schema():
    ...
    ds = Dataset("./data/test_schema_change", schema=schema, shape=(100,), mode="w")
    new_schema_1 = {
        "abc": "uint8",
        "def": {
            "ghi": Tensor((200, 100)),
...
def test_dataset_casting():
    ...
    ds = Dataset(schema=my_schema, url="./data/casting2", shape=(100,), mode="w")
    for i in range(100):
        ds["a", i] = 0.2
    assert ds["a", 30].compute() == np.array([0.2])

    ds2 = Dataset(schema=my_schema, url="./data/casting3", shape=(100,), mode="w")
    ds2["a", 0:100] = np.ones(
        100,
    )
...
def test_dataset_filter():
    ...
    ds = Dataset("./data/new_filter", shape=(10,), schema=my_schema, mode="w")
    for i in range(10):
...
def test_datasetview_filter():
    ...
    my_schema = {"img": Tensor((100, 100)), "ab": Text((None,), max_shape=(10,))}
    ds = Dataset("./data/new_filter_2", shape=(10,), schema=my_schema, mode="w")
    for i in range(10):
        ds["img", i] = i * np.ones((100, 100))
        ds["ab", i] = "abc" + str(i) if i % 2 == 0 else "def" + str(i)
    dsv = ds[2:7]
...
def test_dataset_store():
    ...
    ds = Dataset("./test/ds_store", schema=my_schema, shape=(100,), mode="w")
    for i in range(100):
