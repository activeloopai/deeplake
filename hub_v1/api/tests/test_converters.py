...
import hub_v1.api.tests.test_converters
from hub_v1.schema.features import Tensor
import numpy as np
from hub_v1.utils import tfds_loaded, tensorflow_loaded, pytorch_loaded
import pytest
...
def test_from_tfds_coco():
    ...
    with tfds.testing.mock_data(num_examples=5):
        ds = hub_v1.Dataset.from_tfds("coco", num=5)
...
def test_from_tfds_accentdb():
    ...
    with tfds.testing.mock_data(num_examples=5):
        ds = hub_v1.Dataset.from_tfds("accentdb", num=5)
...
def test_from_tfds_robonet():
    ...
    with tfds.testing.mock_data(num_examples=5):
        ds = hub_v1.Dataset.from_tfds("robonet", num=5)
...
def test_to_tensorflow():
    ...
    ds = hub_v1.Dataset("./data/test_to_tf", shape=(10,), schema=schema, mode="w")
    for i in range(10):
...
def test_to_tensorflow_key_list():
    ...
    ds = hub_v1.Dataset(
        "./data/test_to_tf_key_list", shape=(10,), schema=schema, mode="w"
    )
    for i in range(10):
...
def test_to_pytorch_key_list():
    ...
    ds = hub_v1.Dataset(
        "./data/test_to_pt_key_list", shape=(10,), schema=schema, mode="w"
    )
    for i in range(10):
        ds["abc/d", i] = i * np.ones((100, 100, 3))
        ds["abc/e", i] = i * np.ones((100, 100, 3))
...
def test_to_pytorch_dtype_coversion():
    ...
    ds = hub_v1.Dataset("./data/ds_type_conversion", schema=schema, shape=(10,))
    pt_ds = ds.to_pytorch()
    for item in pt_ds:
        pass
...
def test_to_from_tensorflow_datasetview():
    ...
    out_ds = hub_v1.Dataset.from_tensorflow(tds)
    res_ds = out_ds.store(
...
def test_to_from_pytorch():
    ...
    ds = ds.to_pytorch()
    out_ds = hub_v1.Dataset.from_pytorch(ds)
    with Timer("storing"):
...
def test_to_pytorch_bug():
    ds = hub_v1.Dataset("activeloop/mnist", mode="r")
    data = ds.to_pytorch()


@pytest.mark.skipif(not tensorflow_loaded(), reason="requires tensorflow to be loaded")
def test_to_tensorflow_bug():
    ds = hub_v1.Dataset("activeloop/coco_train")
