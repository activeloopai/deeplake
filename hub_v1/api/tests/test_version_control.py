...
def test_commit_checkout():
    my_schema = {"img": hub_v1.schema.Tensor((1000, 1000, 3))}
    ds = hub_v1.Dataset("./data/eg_1", shape=(10,), schema=my_schema, mode="w")
...
def test_read_mode():
    my_schema = {"abc": "uint8"}
    ds = hub_v1.Dataset("./data/test_versioning/read_ds", schema=my_schema, shape=(10,))
    ds.checkout("second", create=True)
    ds2 = hub_v1.Dataset("./data/test_versioning/read_ds", mode="r")
...
def test_checkout_address_not_found():
    my_schema = {"abc": "uint8"}
    ds = hub_v1.Dataset(
        "./data/test_versioning/ds_address", schema=my_schema, shape=(10,)
    )
...
def test_old_datasets():
    ds = hub_v1.Dataset("activeloop/mnist")
