...
import numpy as np
import pytest
from hub_v1.auto.tests.util import get_dataset_store
from hub_v1.utils import pandas_loaded
from hub_v1.auto.util import get_children
...
def assert_conversion(tag):
    ...
    try:
        ds = hub_v1.Dataset.from_path(str(dataset_store))
    except Exception:
        assert False
    ...
    # df = Pandas dataframe, ds = Dataset obtained from hub_v1.auto
    if df is not None:
