...
import hub_v1
from hub_v1.utils import ray_loaded
from hub_v1.schema import Tensor, Text
import pytest
from hub_v1.compute.ray import empty_remote
...
def test_ray_simple():
    ...
    @hub_v1.transform(schema=schema, scheduler="ray")
    def process(item):
        return {"var": 1}
...
def test_ray_non_dynamic():
    ...
    @hub_v1.transform(schema=schema, scheduler="ray_generator")
    def process(item):
...
def test_ray_pipeline_multiple():
    ...
    @hub_v1.transform(schema=dynamic_schema, scheduler="ray_generator", workers=2)
