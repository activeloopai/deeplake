...
import hub_v1
from hub_v1.schema import Tensor, Image, Text
from hub_v1.utils import Timer
from hub_v1.utils import hub_creds_exist
import pytest
...
def test_threaded():
    ...
    @hub_v1.transform(schema=schema, scheduler="threaded", workers=2)
...
def test_pipeline_dynamic():
    ...
    @hub_v1.transform(schema=dynamic_schema)
...
def test_pipeline_multiple():
    ...
    @hub_v1.transform(schema=dynamic_schema, scheduler="threaded", workers=2)
    def dynamic_transform(sample, multiplier: int = 2):
        return [
            {
                "image": sample["image"] * multiplier,
...
def test_multiprocessing(sample_size=200, width=100, channels=4, dtype="uint8"):
    ...
    with Timer("multiprocesing"):
        ...
        @hub_v1.transform(schema=my_schema, scheduler="threaded", workers=4)
        def my_transform(x):
...
def test_stacked_transform():
    ...
    @hub_v1.transform(schema=schema)
    ...
    ds = hub_v1.Dataset("./data/stacked_transform", mode="w", shape=(5,), schema=schema)
    for i in range(5):
...
def benchmark(sample_size=100, width=1000, channels=4, dtype="int8"):
    ...
    for name, processes in [
        ...
        @hub_v1.transform(schema=my_schema, scheduler=name, processes=processes)
        def my_transform(sample):
            return {
