...
def test_read_write_sequence():
    ...
    data_in = [np_random.rand(125, 125).astype(dtype) for _ in range(10)]
    while data_in:
        ...
        np_testing.assert_array_equal(data_out, data_in[:num_samples])
...
def test_read_write_sequence_big(cat_path):
    ...
    for i in range(50):
        if i % 10 == 0:
            data_in.append(np_random.rand(751, 750, 3).astype(dtype))
        elif i % 3 == 0:
            data_in.append(hub_read(cat_path))
        else:
            data_in.append(np_random.rand(125, 125, 3).astype(dtype))
...
def test_read_write_numpy():
    ...
    common_args["tensor_meta"] = tensor_meta
    dtype = tensor_meta.dtype
    data_in = np_random.rand(10, 125, 125).astype(dtype)
    while len(data_in) > 0:
        chunk = UncompressedChunk(**common_args)
        num_samples = int(chunk.extend_if_has_space(data_in))
        data_out = np_array([chunk.read_sample(i) for i in range(num_samples)])
        if num_samples > 0:
            np_testing.assert_array_equal(data_out, data_in[:num_samples])
        data_in = data_in[num_samples:]
...
def test_read_write_numpy_big():
    ...
    common_args["tensor_meta"] = tensor_meta
    dtype = tensor_meta.dtype
    data_in = np_random.rand(2, 750, 750, 3).astype(dtype)
...
def test_update():
    ...
    common_args["tensor_meta"] = tensor_meta
    dtype = tensor_meta.dtype
    data_in = np_random.rand(7, 125, 125).astype(dtype)
    chunk = UncompressedChunk(**common_args)
    chunk.extend_if_has_space(data_in)

    data_out = np_array([chunk.read_sample(i) for i in range(7)])
    np_testing.assert_array_equal(data_out, data_in)

    data_3 = np_random.rand(175, 175).astype(dtype)
    data_5 = np_random.rand(375, 375).astype(dtype)
    ...
    for i in range(7):
        if i == 3:
            ...
            np_testing.assert_array_equal(chunk.read_sample(i), data_in[i])
