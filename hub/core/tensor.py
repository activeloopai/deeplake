    ...
    def rename(self, key: str):
        ...
        chunk_keys = [
            (
                ...
                get_chunk_key(key, chunk_name, commit_id),
            )
            for chunk_name in chunk_names
        ]

        for old_key, new_key in chunk_keys:
            try:
                self.storage[new_key] = self.storage[old_key]
        ...
        if commit_id != FIRST_COMMIT_ID:
            ...
            enc = chunk_engine.chunk_id_encoder
            for i in range(chunk_engine.num_chunks):
                chunk_name = enc.get_name_for_chunk(i)
                chunk_commit_id = chunk_engine.get_chunk_commit(chunk_name)
                if chunk_commit_id != commit_id:
                    ...
                    if chunk_commit_id != FIRST_COMMIT_ID:
                        chunk_set_key = get_tensor_commit_chunk_set_key(
                            self.key, chunk_commit_id
                        )
                        new_chunk_set_key = get_tensor_commit_chunk_set_key(
                            key, chunk_commit_id
                        )
                        chunks = chunk_engine.meta_cache.get_cachable(
                            chunk_set_key, CommitChunkSet
                        ).chunks
                        chunk_set = CommitChunkSet()
                        chunk_set.chunks = chunks
                        chunk_engine.meta_cache[new_chunk_set_key] = chunk_set

            chunk_set_key = get_tensor_commit_chunk_set_key(self.key, commit_id)
            new_chunk_set_key = get_tensor_commit_chunk_set_key(key, commit_id)
            try:
                chunk_engine.meta_cache[new_chunk_set_key] = chunk_engine.meta_cache[
                    chunk_set_key
                ]
                del chunk_engine.meta_cache[chunk_set_key]
            except KeyError:
                pass

        meta_key = get_tensor_meta_key(self.key, commit_id)
        new_meta_key = get_tensor_meta_key(key, commit_id)
        try:
            self.storage[new_meta_key] = self.storage[meta_key]
            del self.storage[meta_key]
        except KeyError:
            pass

        info_key = get_tensor_info_key(self.key, commit_id)
        new_info_key = get_tensor_info_key(key, commit_id)
        try:
            self.storage[new_info_key] = self.storage[info_key]
            del self.storage[info_key]
        except KeyError:
            pass
        ...
        new_chunk_id_key = get_chunk_id_encoder_key(key, commit_id)
        try:
            self.storage[new_chunk_id_key] = self.storage[chunk_id_key]
            del self.storage[chunk_id_key]
        except KeyError:
            pass

        tile_encoder_key = get_tensor_tile_encoder_key(self.key, commit_id)
        new_tile_encoder_key = get_tensor_tile_encoder_key(key, commit_id)
        try:
            self.storage[new_tile_encoder_key] = self.storage[tile_encoder_key]
            del self.storage[tile_encoder_key]
        except KeyError:
            pass

        self.version_state["full_tensors"][key] = self
        del self.version_state["full_tensors"][self.key]

        ds_meta = self.dataset.meta
        ds_meta.tensors.remove(self.key)
        ds_meta.tensors.append(key)

        self.key = key
        self.chunk_engine.key = key
        self.storage.flush()
