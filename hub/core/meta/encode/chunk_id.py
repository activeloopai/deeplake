...
from hub.constants import ENCODER_NUM_ENTRIES, ENCODING_DTYPE
from hub.util.exceptions import (
    ChunkIdEncoderError,
    ChunkIdEncoderFullError,
...
class ChunkIdEncoder(Encoder, HubMemoryObject):
    ...
    def generate_chunk_id(
        ...
        Raises:
            OutOfChunkCountError: When the row is out of chunk bounds
        """
        if row is None and self.num_chunks >= ENCODER_NUM_ENTRIES:
            raise ChunkIdEncoderFullError

        id = generate_id(self.dtype)
        if register:
            if self.num_samples == 0:
                assert row is None
                self._encoded = np.array([[id, -1]], dtype=self.dtype)
