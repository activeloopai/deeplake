...
from hub import dataset, Dataset
...
def test_commit_checkout(local_ds):
    with local_ds:
        ...
        local_ds.img.extend(np_ones((10, 100, 100, 3)))
        ...
        for i in range(5):
            assert (local_ds.img[i].numpy() == 2 * np_ones((100, 100, 3))).all()
        local_ds.checkout(first_commit_id)  # now all images are ones again

        for i in range(10):
            assert (local_ds.img[i].numpy() == np_ones((100, 100, 3))).all()
        ...
        for i in range(5):
            assert (local_ds.img[i].numpy() == 3 * np_ones((100, 100, 3))).all()
...
def test_commit_checkout_2(local_ds):
    ...
    local_ds.create_tensor("img")
    for i in range(10):
        local_ds.img.append(i * np_ones((100, 100, 3)))
    a = local_ds.commit("first")

    local_ds.img[7] *= 2

    assert (local_ds.img[6].numpy() == 6 * np_ones((100, 100, 3))).all()
    assert (local_ds.img[7].numpy() == 2 * 7 * np_ones((100, 100, 3))).all()
    assert (local_ds.img[8].numpy() == 8 * np_ones((100, 100, 3))).all()
    assert (local_ds.img[9].numpy() == 9 * np_ones((100, 100, 3))).all()

    assert (local_ds.img[2].numpy() == 2 * np_ones((100, 100, 3))).all()
    ...
    assert (local_ds.img[7].numpy() == 7 * np_ones((100, 100, 3))).all()
    ...
    assert (local_ds.img[7].numpy() == 3 * 7 * np_ones((100, 100, 3))).all()
...
def test_dynamic(local_ds):
    ...
    a = local_ds.commit("first")
    for i in range(10):
        local_ds.img[i] = 2 * i * np_ones((150, 150, 3))
    local_ds.checkout(a)

    for i in range(10):
        assert (local_ds.img[i].numpy() == i * np_ones((100, 100, 3))).all()

    local_ds.checkout("main")

    for i in range(10):
        assert (local_ds.img[i].numpy() == 2 * i * np_ones((150, 150, 3))).all()
...
def test_different_lengths(local_ds):
    with local_ds:
        local_ds.create_tensor("img")
        local_ds.create_tensor("abc")
        local_ds.img.extend(np_ones((5, 50, 50)))
        local_ds.abc.extend(np_ones((2, 10, 10)))
        first = local_ds.commit("stored 5 images, 2 abc")
        local_ds.img.extend(np_ones((3, 50, 50)))
        second = local_ds.commit("stored 3 more images")
        assert len(local_ds.tensors) == 2
        assert len(local_ds.img) == 8
        assert (local_ds.img.numpy() == np_ones((8, 50, 50))).all()
        assert len(local_ds.abc) == 2
        assert (local_ds.abc.numpy() == np_ones((2, 10, 10))).all()
        local_ds.checkout(first)
        assert len(local_ds.tensors) == 2
        assert len(local_ds.img) == 5
        assert (local_ds.img.numpy() == np_ones((5, 50, 50))).all()
        assert len(local_ds.abc) == 2
        assert (local_ds.abc.numpy() == np_ones((2, 10, 10))).all()

        # will autocheckout to new branch
        local_ds.create_tensor("ghi")
        local_ds.ghi.extend(np_ones((2, 10, 10)))
        local_ds.img.extend(np_ones((2, 50, 50)))
        local_ds.abc.extend(np_ones((3, 10, 10)))
        assert len(local_ds.tensors) == 3
        assert len(local_ds.img) == 7
        assert (local_ds.img.numpy() == np_ones((7, 50, 50))).all()
        assert len(local_ds.abc) == 5
        assert (local_ds.abc.numpy() == np_ones((5, 10, 10))).all()
        assert len(local_ds.ghi) == 2
        assert (local_ds.ghi.numpy() == np_ones((2, 10, 10))).all()
