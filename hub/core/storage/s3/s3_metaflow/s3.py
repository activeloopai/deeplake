...
class S3(object):
    ...
    def __init__(self,
        ...
        if not boto_found:
            raise Exception("You need to install 'boto3' in order to use S3.")

        if s3root:
            # 2. use an explicit S3 prefix
            parsed = urlparse(to_unicode(s3root))
            if parsed.scheme != 's3':
                raise Exception("s3root needs to be an S3 URL prefxied with s3://.")
        ...
        def _check():
            for key, path in key_paths:
                if not os.path.exists(path):
                    raise Exception("Local file not found: %s" % path)
    ...
    def _one_boto_op(self, op, url):
        ...
        for i in range(NUM_S3OP_RETRIES):
            ...
            try:
                ...
                if error_code == 404:
                    ...
                    raise Exception("Specified S3 bucket doesn't exist.")
                error = str(err)
                print("1", err)
        ...
        raise Exception("S3 operation failed.\n"\
                                  "Key requested: %s\n"\
                                  "Error: %s" % (url, error))
    ...
    def _read_many_files(self, op, prefixes, **options):
        ...
        with NamedTemporaryFile(dir=self._tmpdir,
            ...
            stdout, stderr = self._s3op_with_retries(op,
                                                     ...
                                                     **options)
            if stderr:
                raise Exception("Getting S3 files failed.\n"\
    ...
    def _put_many_files(self, url_files, overwrite):
        ...
        with NamedTemporaryFile(dir=self._tmpdir,
            ...
            stdout, stderr = self._s3op_with_retries('put',
                                                ...
                                                listing=True)
            if stderr:
                raise S3Exception("Uploading S3 files failed.\n"\
                                          "First key: %s\n"\
