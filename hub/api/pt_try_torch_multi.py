...
def _read_chunks(chunk_path):
    storage = s3_client()
    a = torch.Tensor.share_memory_(storage[chunk_path])
    return chunk_path.split("/")[-1], a
...
class TorchDataset:
    def __init__(self, ds, transform=None, workers=1):
        ...
        self.map = Pool(processes=workers).map
    ...
    def _get_data_from_chunks(self, start_ind, key, chunk_map):
    ...
    def __getitem__(self, index):
        for key in self.ds.tensors:
            ...
            ls = self.map(_read_chunks, chunk_paths)
            chunk_dict = dict(ls)
            self._get_data_from_chunks(index, key, chunk_dict)
