...
class dataset:
    def __new__(
        ...
        Args:
            ...
            s3_profile_name (str, optional): The name of the AWS profile to use. Applicable only for s3 paths. This is relevant when you want to use profile other than default while reading creds from aws credentials file. This is ignored when 'creds' is passed.
    ...
    def empty(
        ...
        Args:
            ...
            creds (dict, optional): A dictionary containing credentials used to access the dataset at the path.
                This takes precedence over credentials present in the environment. Currently only works with s3 paths.
                It supports 'aws_access_key_id', 'aws_secret_access_key', 'aws_session_token', 'endpoint_url' and 'region' as keys.
            s3_profile_name: The name of the AWS profile to use. Applicable only for s3 paths. This is relevant when you want to use profile other than default while reading creds from aws credentials file. This is ignored when 'creds' is passed.
            token (str, optional): Activeloop token, used for fetching credentials to the dataset at path if it is a Hub dataset. This is optional, tokens are normally autogenerated.
