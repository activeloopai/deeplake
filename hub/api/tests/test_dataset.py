...
from hub.util.exceptions import AuthorizationException, DatasetHandlerError


def test_new_dataset():
    with CliRunner().isolated_filesystem():
        ds = hub_dataset("test_new_dataset")
        with ds:
            ds.create_tensor("image")
            for i in range(10):
                ds.image.append(i * np_ones((100 * (i + 1), 100 * (i + 1))))

        for i in range(10):
            np_testing.assert_array_equal(
                ds.image[i].numpy(), i * np_ones((100 * (i + 1), 100 * (i + 1)))
            )
...
def test_dataset_empty_load():
    with CliRunner().isolated_filesystem():
        ...
        with pytest.raises(DatasetHandlerError):
            ds_empty = hub_empty(path)

        ds_loaded = hub_load(path)
        for i in range(10):
            np_testing.assert_array_equal(
        ...
        with pytest.raises(DatasetHandlerError):
            ds_random = hub_load("some_random_path")

        ds_overwrite_load = hub_dataset(path, overwrite=True)
        ...
        with ds_overwrite_load:
            ...
            for i in range(10):
                ds_overwrite_load.image.append(
                    i * np_ones((100 * (i + 1), 100 * (i + 1)))
                )

        with pytest.raises(DatasetHandlerError):
            ds_empty = hub_empty(path)

        ds_overwrite_empty = hub_dataset(path, overwrite=True)
...
def test_persistence_bug(local_ds_generator):
    for tensor_name in ["abc", "abcd/defg"]:
        ...
        ds = local_ds_generator()
        np_testing.assert_array_equal(ds[tensor_name].numpy(), np_array([[1], [2]]))
