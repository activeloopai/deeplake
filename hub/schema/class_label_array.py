...
class ClassLabelArray(Tensor):
    """`HubSchema` for list of integer class labels."""
    ...
    def __init__(
        ...
        shape: tuple of ints or None
            The shape of ClassLabelArray:
            (num_class_labels,) where num_class_labels can be None.
            Defaults to (None,).
        max_shape : Tuple[int]
    ...
    def _check_shape(self, shape):
        """Check if provided shape matches ClassLabelArray characteristics."""
        if len(shape) != 1:
            raise ValueError(
                "Wrong ClassLabelArray shape provided, should be of the format (num_class_labels,) where num_class_labels is an integer or None"
            )