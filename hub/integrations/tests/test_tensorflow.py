...
def test_tensorflow_with_compression(local_ds: Dataset):
    ...
    images.extend(np_ones((16, 10, 10, 3), dtype="uint8"))
...
def test_tensorflow_small(local_ds):
    ...
    local_ds.image.extend(np_array([i * np_ones((10, 10)) for i in range(256)]))
    local_ds.create_tensor("image2")
    local_ds.image2.extend(np_array([i * np_ones((12, 12)) for i in range(256)]))
    local_ds.flush()

    tds = local_ds.tensorflow()
    for i, batch in enumerate(tds):
        # converting tf Tensors to numpy
        np_testing.assert_array_equal(batch["image"].numpy(), i * np_ones((10, 10)))
        np_testing.assert_array_equal(batch["image2"].numpy(), i * np_ones((12, 12)))
...
def test_groups(local_ds, compressed_image_paths):
    ...
    tds = local_ds.tensorflow()
    for batch in tds:
        np_testing.assert_array_equal(batch["images/jpegs/cats"].numpy(), img1.array)
        np_testing.assert_array_equal(batch["images/pngs/flowers"].numpy(), img2.array)
    ...
    for batch in tds:
        np_testing.assert_array_equal(batch["jpegs/cats"].numpy(), img1.array)
        np_testing.assert_array_equal(batch["pngs/flowers"].numpy(), img2.array)
