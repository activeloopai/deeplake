...
class HubTensorflowDataset(tf.data.Dataset):
  ...
  def __init__(self):
    try:
      ...
      raise AttributeError("{}: A likely cause of this error is that the super "
                           ...
                           "super call.".format(e))
    super(HubTensorflowDataset, self).__init__(variant_tensor)
  ...
  def from_sparse_tensor_slices(sparse_tensor):
    ...
    """
    return DatasetAdapter(SparseTensorSliceDataset(sparse_tensor))
  ...
  def concatenate(self, dataset):
    return DatasetAdapter(super(HubTensorflowDataset, self).concatenate(dataset))

  @functools.wraps(tf.data.Dataset.prefetch)
  def prefetch(self, buffer_size):
    return DatasetAdapter(super(HubTensorflowDataset, self).prefetch(buffer_size))
  ...
  def repeat(self, count=None):
    return DatasetAdapter(super(HubTensorflowDataset, self).repeat(count))

  @functools.wraps(tf.data.Dataset.shuffle)
  def shuffle(self, buffer_size, seed=None, reshuffle_each_iteration=None):
    return DatasetAdapter(super(HubTensorflowDataset, self).shuffle(
        buffer_size, seed, reshuffle_each_iteration))

  @functools.wraps(tf.data.Dataset.cache)
  def cache(self, filename=""):
    return DatasetAdapter(super(HubTensorflowDataset, self).cache(filename))

  @functools.wraps(tf.data.Dataset.take)
  def take(self, count):
    return DatasetAdapter(super(HubTensorflowDataset, self).take(count))

  @functools.wraps(tf.data.Dataset.skip)
  def skip(self, count):
    return DatasetAdapter(super(HubTensorflowDataset, self).skip(count))

  @functools.wraps(tf.data.Dataset.shard)
  def shard(self, num_shards, index):
    return DatasetAdapter(super(HubTensorflowDataset, self).shard(num_shards, index))
  ...
  def batch(self,
            ...
            deterministic=None):
    return DatasetAdapter(
        super(HubTensorflowDataset, self).batch(batch_size, drop_remainder,
  ...
  def padded_batch(self,
                   ...
                   drop_remainder=False):
    return DatasetAdapter(
        super(HubTensorflowDataset, self).padded_batch(batch_size, padded_shapes,
  ...
  def flat_map(self, map_func):
    return DatasetAdapter(super(HubTensorflowDataset, self).flat_map(map_func))
  ...
  def interleave(self,
                 ...
                 deterministic=None):
    return DatasetAdapter(
        super(HubTensorflowDataset, self).interleave(map_func, cycle_length, block_length,
                                          num_parallel_calls, deterministic))

  @functools.wraps(tf.data.Dataset.filter)
  def filter(self, predicate):
    return DatasetAdapter(super(HubTensorflowDataset, self).filter(predicate))
  ...
  def apply(self, transformation_func):
    return DatasetAdapter(super(HubTensorflowDataset, self).apply(transformation_func))

  @functools.wraps(tf.data.Dataset.window)
  def window(self, size, shift=None, stride=1, drop_remainder=False):
    return DatasetAdapter(super(HubTensorflowDataset, self).window(
        size, shift, stride, drop_remainder))

  @functools.wraps(tf.data.Dataset.unbatch)
  def unbatch(self):
    return DatasetAdapter(super(HubTensorflowDataset, self).unbatch())

  @functools.wraps(tf.data.Dataset.with_options)
  def with_options(self, options):
    return DatasetAdapter(super(HubTensorflowDataset, self).with_options(options))
