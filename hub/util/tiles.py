...
def merge_tiles_into_sample_array(
    sample_shape: Tuple[int, ...], dtype, tiles
) -> np.ndarray:
    ...
    for tile_index, tile_obj in np.ndenumerate(tiles):
        ...
        for dim, index in enumerate(tile_index):
            ...
            sum_shapes = sum(
                [
                    tiles[tuple(el)].shape[dim]
                    for el in indices
                    if tiles[tuple(el)] is not None
                ]
            )
            ...
            sample_view.append(
                slice(
                    sum_shapes, min(sum_shapes + tile_obj.shape[dim], sample.shape[dim])
                )
            )

            # clamp tile view in case it goes out of borders
            tile_view.append(
                slice(0, min(tile_obj.shape[dim], sample.shape[dim] - sum_shapes))
            )
