...
def bbox_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
    try:
...
def radio_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
...
def checkbox_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
...
def point_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
    try:
...
def line_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
    try:
...
def raster_segmentation_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
    try:
        ds.create_tensor(tensor_name, htype='segment_mask', dtype='uint8', sample_compression="lz4")
    except:
        pass
    ...
    def mask_converter(row, obj):
        try:
            ...
            with urllib.request.urlopen(r) as response:
                ...
                ds[tensor_name][row] = mask[..., np.newaxis]
...
def text_converter_(obj, converter, tensor_name, context):
    ds = context['ds']
    try:
