...
def validate_ds(ds):
    ...
    "bbox/bbox", "bbox/fully_visible", "checklist", "frame_idx", "frames",
    "line", "mask/mask", "mask/mask_label", "mask/mask_labels",
    "metadata/current_frame_name", "metadata/data_row_id", "metadata/dataset_id",
    "metadata/dataset_name", "metadata/frame_count", "metadata/frame_number",
    "metadata/frame_rate", "metadata/global_key", "metadata/height",
    "metadata/label_creator", "metadata/name", "metadata/ontology_id",
    "metadata/project_name", "metadata/width", "point", "radio_bttn",
    "radio_bttn_scale", "text", "video_idx"
    })

    assert np.all(ds["bbox/bbox"][0:3].numpy() == [
    [[1096,    9,  362,  369]],
    [[1096,    8,  362,  368]],
    [[1097,    8,  362,  368]]
    ])
    ...
    assert np.all(ds["frame_idx"][245:255].numpy() == [[245],
       [246],
       [247],
       [248],
       [249],
    ...
    assert np.all(ds["frame_idx"][495:505].numpy() == [[495],
       [496],
       [497],
       [498],
       [499],
    ...
    assert np.all(ds["metadata/current_frame_name"][245:255].numpy() == [['output005_000245'],
       ['output005_000246'],
       ['output005_000247'],
       ['output005_000248'],
       ['output005_000249'],
    ...
    assert np.all(ds["metadata/current_frame_name"][495:505].numpy() == [['output005_000495'],
       ['output005_000496'],
       ['output005_000497'],
       ['output005_000498'],
       ['output005_000499'],
    ...
    assert np.all(ds["video_idx"][245:255].numpy() == [[0],
       [0],
       [0],
       [0],
       [0],
    ...
    assert np.all(ds["video_idx"][495:505].numpy() == [[0],
       [0],
       [0],
       [0],
       [0],
...
def test_connect_to_labelbox():
    ...
    ds = create_dataset_from_video_annotation_project(ds_path, project_id, client, API_KEY, deeplake_token=os.environ["MY_ACTIVELOOP_PROD_TOKEN"], overwrite=True, url_presigner=url_presigner)
    def ds_provider(p):
