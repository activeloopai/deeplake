...
def test_corrupt_dataset(local_ds, corrupt_image_paths, compressed_image_paths):
    img_good = dl.read(compressed_image_paths["jpeg"][0])
    img_bad = dl.read(corrupt_image_paths["jpeg"])
    with local_ds:
...
def test_groups(local_ds, compressed_image_paths):
    img1 = dl.read(compressed_image_paths["jpeg"][0])
    img2 = dl.read(compressed_image_paths["png"][0])
    with local_ds:
...
def test_pytorch_tobytes(local_ds, compressed_image_paths, compression):
    ...
    with ds:
        ...
        ds.image.extend([dl.read(compressed_image_paths["jpeg"][0])] * 5)
