...
>>> ds = dl.empty("hub://fayazrahman4u/my_dataset") # create dataset
>>> ds.create_tensor("images", htype="image", sample_compression="jpg") # create a tensor
>>> ds.images.append(dl.read("files/images/dog.jpg")) # add a sample
>>> ds.commit("creation") # commit -> trigger logging
>>> run.finish()

NOTE:
    If you created your dataset using :meth:`dl.deepcopy`, perform the commit only if you have head changes.
...
~~~~~~~~~~~~~~~~~~~~
A dataset read will be logged if you iterate over a dataset or call :meth:`Dataset.pytorch() <dl.core.dataset.Dataset.pytorch>` 
or :meth:`Tensor.numpy() <dl.core.tensor.Tensor.numpy>` on its tensors.
...
def wandb_run():
    if not dl.constants.WANDB_INTEGRATION_ENABLED:
        return
