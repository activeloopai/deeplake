...
def test_rechunk_link(local_ds_generator, cat_path, flower_path, color_image_paths):
    ...
    with local_ds_generator() as ds:
        ...
        add_sample_in().eval(
            [dl.link(dog_path), dl.link(flower_path), dl.link(cat_path)],
            ds,
            num_workers=2,
        )
        assert len(ds.abc.chunk_engine.chunk_id_encoder.array) == 2
        ds.abc[0] = dl.link(cat_path)
...
def test_rechunk_cloud_link(local_ds_generator):
    ...
    with local_ds_generator() as ds:
        ...
        add_sample_in().eval([dl.link(s3_path_1, "my_s3_key_1")] * 3, ds, num_workers=2)
        assert len(ds.abc.chunk_engine.chunk_id_encoder.array) == 2
        ds.abc[0] = dl.link(s3_path_2, "my_s3_key_2")
