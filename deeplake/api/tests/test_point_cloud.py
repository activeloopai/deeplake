...
def test_point_cloud(local_ds, point_cloud_paths):
    ...
    local_ds.point_cloud_without_sample_compression.append(dl.read(path))
    ...
    with pytest.raises(CorruptedSampleError):
        local_ds.point_cloud_with_sample_compression.append(
            dl.read(get_dummy_data_path("point_cloud/corrupted_point_cloud.las"))
        )

    local_ds.point_cloud_with_sample_compression.append(dl.read(path, verify=True))
...
def test_point_cloud_slicing(local_ds: Dataset, point_cloud_paths):
    for compression, path in point_cloud_paths.items():
        ...
        local_ds.point_cloud.append(dl.read(path))
