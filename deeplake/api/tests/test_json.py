    ...
    items = [
        {
            "x": [1, 2, 3],
            "y": [4, [5, 6]],
            "z": dl.read(compressed_image_paths["jpeg"][0]),
        },
        {
            "x": [1, 2, 3],
            "y": [4, {"z": [0.1, 0.2, []]}],
            "z": dl.read(compressed_image_paths["png"][0]),
        },
    ]
    with ds:
        for x in items:
...
def test_list_with_hub_sample(memory_ds, compressed_image_paths):
    ...
    items = [
        [
            {
                "x": [1, 2, 3],
                "y": [4, [5, 6, dl.read(compressed_image_paths["jpeg"][0])]],
            },
            [[dl.read(compressed_image_paths["jpeg"][1])]],
            [None, 0.1],
        ],
        [
            [],
            [[[dl.read(compressed_image_paths["png"][0])]]],
            {"a": [0.1, 1, "a", dl.read(compressed_image_paths["png"][0])]},
        ],
    ]
    with ds:
        for x in items:
