...
@pytest.mark.parametrize("compression", dl.compression.VIDEO_COMPRESSIONS)
def test_video(local_ds, compression, video_paths):
    for i, path in enumerate(video_paths[compression]):
        ...
        with local_ds:
            for _ in range(5):
                tensor.append(dl.read(path))  # type: ignore
            tensor.extend([dl.read(path) for _ in range(5)])  # type: ignore
        for i in range(10):
...
def test_video_slicing(local_ds: Dataset, video_paths):
    for path in video_paths["mp4"]:
        if "samplemp4_1MB" in path:
            ...
            local_ds.video.append(dl.read(path))
...
def test_video_streaming(vstream_path, hub_token):
    ds = dl.load(vstream_path, read_only=True, token=hub_token)
...
def test_video_sequence(local_ds, video_paths):
    with local_ds as ds:
        ...
        ds.video_seq.append([dl.read(video_paths["mp4"][0]) for _ in range(3)])
        ds.video_seq.append([dl.read(video_paths["mp4"][1]) for _ in range(3)])
...
def test_video_data(local_ds, video_paths):
    with local_ds as ds:
        ...
        for _ in range(3):
            ds.video.append(dl.read(video_paths["mp4"][0]))
        ds.video.append(dl.read(video_paths["mp4"][1]))
