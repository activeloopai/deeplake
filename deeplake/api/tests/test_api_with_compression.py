...
def _populate_compressed_samples(tensor: Tensor, cat_path, flower_path, count=1):
    for _ in range(count):
        tensor.append(dl.read(cat_path))
        tensor.append(dl.read(flower_path))
...
def test_chunkwise_compression(memory_ds, cat_path, flower_path):
    ...
    with ds:
        ...
        images.extend([dl.read(flower_path)] * im_ct)
        expected_arr = np.random.randint(0, 256, (200, 250, 4)).astype("uint8")
        images.append(expected_arr)
        images.extend([dl.read(flower_path)] * im_ct)
        expected_img = np.array(dl.read(flower_path))
    ds.clear_cache()
    for i, img in enumerate(images):
        if i == im_ct:
            assert_images_close(img.numpy(), expected_arr)
...
@pytest.mark.parametrize("compression", dl.compression.AUDIO_COMPRESSIONS)
def test_audio(local_ds, compression, audio_paths):
    ...
    with local_ds:
        for _ in range(10):
            local_ds.audio.append(dl.read(path))  # type: ignore
    for i in range(10):
...
def test_forced_htypes(
    ...
    with memory_ds as ds:
        ...
        gray.append(dl.read(grayscale_image_paths["jpeg"]))
        gray.append(dl.read(color_image_paths["jpeg"]))
        gray.append(dl.read(flower_path))
        gray.extend(np.ones((4, 10, 10, 3), dtype=np.uint8))
        gray.extend(
            [dl.read(color_image_paths["jpeg"]), np.ones((10, 10), dtype=np.uint8)]
        )

        rgb.append(dl.read(grayscale_image_paths["jpeg"]))
        rgb.append(dl.read(color_image_paths["jpeg"]))
        rgb.append(dl.read(flower_path))
        rgb.extend(np.ones((4, 10, 10), dtype=np.uint8))
        rgb.extend(
            [
                dl.read(grayscale_image_paths["jpeg"]),
        ...
        gray_png.append(dl.read(flower_path))
        gray_png.append(np.ones((10, 10, 4), dtype=np.uint8))

        rgb_png.append(dl.read(flower_path))
