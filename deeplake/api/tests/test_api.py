...
def test_persist(ds_generator):
    ...
    assert ds_new.meta.version == dl.__version__
...
def test_populate_dataset(local_ds):
    ...
    assert local_ds.meta.version == dl.__version__
...
def test_htype(memory_ds: Dataset):
    ...
    point_cloud.append(
        dl.read(os.path.join(get_dummy_data_path("point_cloud"), "point_cloud.las"))
    )
    point_cloud_dummy_data_path = pathlib.Path(get_dummy_data_path("point_cloud"))
    point_cloud.append(dl.read(point_cloud_dummy_data_path / "point_cloud.las"))
...
def test_hub_dataset_suffix_bug(hub_cloud_ds, hub_cloud_dev_token):
    ...
    ds = dl.dataset(hub_cloud_ds.path[:-1], token=hub_cloud_dev_token)
...
def test_like(local_path, convert_to_pathlib):
    ...
    dest_ds = dl.like(dest_path, src_ds)
...
def test_dataset_delete():
    with CliRunner().isolated_filesystem():
        ...
        dl.delete("test/", force=True)
        assert not os.path.isfile("test/test.txt")

        dl.empty("test/").create_tensor("tmp")
        ...
        dl.empty(pathlib_path).create_tensor("tmp")
        assert os.path.isfile("test/dataset_meta.json")

        dl.delete(pathlib_path)
        assert not os.path.isfile("test/dataset_meta.json")

        old_size = dl.constants.DELETE_SAFETY_SIZE
        dl.constants.DELETE_SAFETY_SIZE = 1 * MB
        ...
        dl.delete("test/", large_ok=True)
        assert not os.path.isfile("test/dataset_meta.json")

        dl.constants.DELETE_SAFETY_SIZE = old_size
...
def test_dataset_rename(ds_generator, path, hub_token, convert_to_pathlib):
    ...
    ds = dl.rename(ds.path, new_path, token=hub_token)
    assert ds.path == str(new_path)
    np.testing.assert_array_equal(ds.abc.numpy(), np.array([[1, 2, 3, 4]]))

    ds = dl.load(new_path, token=hub_token)
    np.testing.assert_array_equal(ds.abc.numpy(), np.array([[1, 2, 3, 4]]))

    with pytest.raises(InvalidTokenException):
        ds = dl.load("hub://activeloop-test/sohas-weapons-train", token="invalid token")
    ...
    dl.delete(new_path, token=hub_token)
...
def test_dataset_deepcopy(path, hub_token, num_workers, progressbar):
    ...
    src_ds = dl.empty(src_path, overwrite=True, token=hub_token)
    ...
    with pytest.raises(DatasetHandlerError):
        dl.deepcopy(src_path, dest_path, src_token=hub_token, dest_token=hub_token)
    ...
    dest_ds = dl.load(dest_path, token=hub_token)
    assert list(dest_ds.tensors) == ["a", "b", "c", "d"]
    ...
    for tensor in dest_ds.tensors:
        np.testing.assert_array_equal(src_ds[tensor].numpy(), dest_ds[tensor].numpy())
    dl.delete(src_path, token=hub_token)
    dl.delete(dest_path, token=hub_token)
...
def test_cloud_delete_doesnt_exist(hub_cloud_path, hub_cloud_dev_token):
    ...
    dl.delete(new_path, token=hub_cloud_dev_token, force=True)
...
def test_tobytes(memory_ds, compressed_image_paths, audio_paths):
    ...
    with ds:
        for _ in range(3):
            ds.image.append(dl.read(compressed_image_paths["jpeg"][0]))
            ds.audio.append(dl.read(audio_paths["mp3"]))
...
def test_tobytes_link(memory_ds):
    with memory_ds as ds:
        ...
        ds.images.append(dl.link("https://picsum.photos/id/237/200/300"))
        sample = dl.read("https://picsum.photos/id/237/200/300")
...
def test_ds_append_with_ds_view():
    ds1 = dl.dataset("mem://x")
    ds2 = dl.dataset("mem://y")
    ds1.create_tensor("x")
    ds2.create_tensor("x")
    ds1.create_tensor("y")
    ds2.create_tensor("y")
...
def test_append_with_tensor(src_args, dest_args, size):
    ds1 = dl.dataset("mem://ds1")
    ds2 = dl.dataset("mem://ds2")
...
def test_hub_remote_read_images(storage, memory_ds, color_image_paths, gdrive_creds):
    ...
    image = dl.read("https://picsum.photos/200/300")
...
def test_hub_remote_read_gdrive_root(request, memory_ds, gdrive_creds):
    if not is_opt_true(request, GDRIVE_OPT):
        pytest.skip()
    memory_ds.create_tensor("images", htype="image", sample_compression="jpg")
    memory_ds.images.append(dl.read("gdrive://cat.jpeg", creds=gdrive_creds))
...
def test_sequence_htype_with_hub_read(local_ds, shape, compressed_image_paths):
    ds = local_ds
    imgs = list(map(dl.read, compressed_image_paths["jpeg"][:3]))
...
def test_dataset_copy(
    ...
    local_ds = dl.load(local_ds.path)
...
def test_hub_exists(ds_generator, path, hub_token, convert_to_pathlib):
    ...
    ds = ds_generator()
    assert dl.exists(path, token=hub_token) == True
    assert dl.exists(f"{path}_does_not_exist", token=hub_token) == False


def test_pyav_not_installed(local_ds, video_paths):
    pyav_installed = dl.core.compression._PYAV_INSTALLED
    dl.core.compression._PYAV_INSTALLED = False
    local_ds.create_tensor("videos", htype="video", sample_compression="mp4")
    with pytest.raises(dl.util.exceptions.CorruptedSampleError):
        local_ds.videos.append(dl.read(video_paths["mp4"][0]))
    dl.core.compression._PYAV_INSTALLED = pyav_installed
...
def test_access_method(s3_ds_generator):
    with pytest.raises(DatasetHandlerError):
        dl.dataset("./some_non_existent_path", access_method="download")

    with pytest.raises(DatasetHandlerError):
        dl.dataset("./some_non_existent_path", access_method="local")
...
def test_text_label(local_ds_generator):
    with local_ds_generator() as ds:
        ...
        temp = dl.constants._ENABLE_RANDOM_ASSIGNMENT
        dl.constants._ENABLE_RANDOM_ASSIGNMENT = True
        ...
        dl.constants._ENABLE_RANDOM_ASSIGNMENT = temp
...
def token_permission_error_check(
    ...
    with pytest.raises(TokenPermissionError):
        dl.empty("hub://activeloop-test/sohas-weapons-train")

    with pytest.raises(TokenPermissionError):
        ds = dl.load("hub://activeloop/fake-path")


def invalid_token_exception_check():
    with pytest.raises(InvalidTokenException):
        ds = dl.empty("hub://adilkhan/demo", token="invalid_token")


def user_not_logged_in_exception_check(runner):
    runner.invoke(logout)
    with pytest.raises(UserNotLoggedInException):
        ds = dl.load("hub://activeloop-test/sohas-weapons-train", read_only=True)


def dataset_handler_error_check(runner, username, password):
    result = runner.invoke(login, f"-u {username} -p {password}")
    with pytest.raises(DatasetHandlerError):
        ds = dl.load(f"hub://{username}/wrong-path")
