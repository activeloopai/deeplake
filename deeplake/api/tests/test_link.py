...
def test_none_used_key(local_ds_generator, cat_path):
    ...
    with local_ds as ds:
        ...
        ds.add_creds_key("my_s3_key")
        ds.populate_creds("my_s3_key", {})
        ds.xyz.append(dl.link(cat_path))
        assert ds.link_creds.used_creds_keys == set()
        ds.xyz.append(dl.link(cat_path, "ENV"))
        assert ds.link_creds.used_creds_keys == set()
        ds.xyz.append(dl.link(cat_path, "my_s3_key"))
...
def test_jwt_link(local_ds):
    with local_ds as ds:
        ...
        for _ in range(3):
            ds.img.append(dl.link(img_url, creds_key="my_jwt_key"))
...
def test_complex_creds(local_ds_generator):
    ...
    with local_ds as ds:
        ...
        for i in range(10):
            ...
            sample = dl.link("https://picsum.photos/200/300", creds_key=creds_key)
        ...
        for i in range(10, 15):
            sample = dl.link("https://picsum.photos/200/300")
            ds.link.append(sample)
            ds.xyz.append(i)
...
def test_transform(local_ds, cat_path, flower_path):
    data_in = dl.dataset("./test/link_transform", overwrite=True)
    with data_in as ds:
        ds.create_tensor(
            "linked_images",
            htype="link[image]",
        ...
        for i in range(10):
            sample = dl.link(cat_path) if i % 2 == 0 else dl.link(flower_path)
            ds.linked_images.append(sample)
...
def transform_path_link(sample_in, samples_out):
    samples_out.images.append(dl.link(sample_in))
...
def test_link_managed(hub_cloud_ds_generator, cat_path):
    ...
    with hub_cloud_ds_generator() as ds:
        ...
        ds.img.append(dl.link(cat_path, creds_key=key_name))
...
def test_link_ready(local_ds_generator, cat_path):
    with local_ds_generator() as ds:
        ...
        ds.img.append(dl.link(cat_path, creds_key="abc"))
