...
def test_hub_read(local_ds_generator, images_compression, cat_path, flower_path):
    ...
    ds.images[0] = dl.read(cat_path)
    np.testing.assert_array_equal(ds.images[0].numpy(), dl.read(cat_path).array)

    ds.images[1] = dl.read(flower_path)
    np.testing.assert_array_equal(ds.images[1].numpy(), dl.read(flower_path).array)

    ds.images[8:10] = [dl.read(cat_path), dl.read(flower_path)]
    assert_array_lists_equal(
        ds.images[8:10].numpy(aslist=True),
        [dl.read(cat_path).array, dl.read(flower_path).array],
    )
...
def test_sequence_htype_with_hub_read(local_ds, shape, compressed_image_paths):
    ...
    new_imgs = list(map(dl.read, compressed_image_paths["jpeg"][3:6]))
