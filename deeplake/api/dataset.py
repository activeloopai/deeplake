...
class dataset:
    ...
    def init(
        ...
        """Returns a :class:`~dl.core.dataset.Dataset` object referencing either a new or existing dataset.

        Examples:

            >>> ds = dl.dataset("hub://username/dataset")
            >>> ds = dl.dataset("s3://mybucket/my_dataset")
            >>> ds = dl.dataset("./datasets/my_dataset", overwrite=True)
    ...
    def empty(
        ...
        if overwrite and dataset_exists(cache_chain):
            ...
            raise DatasetHandlerError(
                ...
                f"If you want to load the dataset that exists at this path, use dl.load() instead."
            )
    ...
    def load(
        ...
        if not dataset_exists(cache_chain):
            raise DatasetHandlerError(
                f"A Hub dataset does not exist at the given path ({path}). Check the path provided or in case you want to create a new dataset, use dl.empty()."
            )
    ...
    def rename(
        ...
        Examples:
            ...
            >>> dl.rename("hub://username/image_ds", "hub://username/new_ds")
            >>> dl.rename("s3://mybucket/my_ds", "s3://mybucket/renamed_ds")
        ...
        ds = dl.load(old_path, verbose=False, token=token, creds=creds)
        ds.rename(new_path)
    ...
    def delete(
        ...
        try:
            ...
            ds = dl.load(path, verbose=False, token=token, creds=creds)
    ...
    def copy(
        ...
        if isinstance(src, (str, pathlib.Path)):
            src = convert_pathlib_to_string_if_needed(src)
            src_ds = dl.load(src, read_only=True, creds=src_creds, token=src_token)
        else:
            src_ds = src
    ...
    def ingest(
        ...
        **dataset_kwargs,
        ...
        if isinstance(src, str):
            ...
            ds = dl.dataset(dest, creds=dest_creds, **dataset_kwargs)
    ...
    def ingest_kaggle(
        ...
        **dataset_kwargs,
        ...
        Args:
            ...
            **dataset_kwargs: Any arguments passed here will be forwarded to the dataset creator function. See :func:`dl.dataset`.
