...
class YoloDataset(UnstructuredDataset):
    def __init__(
        ...
        self._initialize_settings(
            image_settings or {}, label_settings or {}, coordinates_settings or {}
        )
    ...
    def _initialize_settings(
        self, image_settings, label_settings, coordinates_settings
    ):
    ...
    def _add_annotation_tensors(
        ...
        structure.add_first_level_tensor(
            TensorStructure(
                name=self.label_settings["name"],
                params={
                    i: self.label_settings[i]
                    for i in self.label_settings
                    if i != "name"
                },
            )
        )
        ...
        structure.add_first_level_tensor(
            TensorStructure(
                name=self.coordinates_settings["name"],
                params={
                    i: self.coordinates_settings[i]
                    for i in self.coordinates_settings
                    if i != "name"
                },
            )
        )
    ...
    def _ingest_data(self, ds: Dataset, progressbar: bool = True, num_workers: int = 0):
            ...
            sample_out.append(
                {
                    ...
                    self.coordinates_settings["name"]: yolo_coordinates.astype(
        ...
        if tensor_meta["coordinates"].htype == "bbox":
            append_data_bbox(tensor_meta=tensor_meta, creds_key=creds_key).eval(
                self.ingestion_data,
                ds,
            ...
            append_data_polygon(tensor_meta=tensor_meta, creds_key=creds_key).eval(
                self.ingestion_data,
                ds,
                progressbar=progressbar,
                num_workers=num_workers,
    ...
    def structure(self, ds: Dataset, progressbar: bool = True, num_workers: int = 0):  # type: ignore
        ...
        if ds[self.coordinates_settings["name"]].meta.htype == "bbox":
            ds[self.coordinates_settings["name"]].info["coords"] = {
                "type": "fractional",
                "mode": "CCWH",
            }
        ...
        if self.verify_class_names and self.data.class_names:
            ...
            if (
                max_label
                != len(ds[self.label_settings.get("name")].info.class_names) - 1
            ):
                raise IngestionError(
