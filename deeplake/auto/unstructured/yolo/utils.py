...
class YoloData:
    def __init__(self, data_directory: Union[str, pathlib.Path], annotations_directory: Union[str, pathlib.Path], class_names_file: Union[str, pathlib.Path], creds) -> None:
        ...
        if annotations_directory:
            ...
            self.root_annotations = convert_pathlib_to_string_if_needed(annotations_directory)
            self.provider_annotations = storage_provider_from_path(self.root_annotations, creds=creds)
        else:
        ...
        self.class_names = self.parse_class_names(class_names_file) if class_names_file else None
    ...
    def read_yolo_coordinates(self, file_name: str, is_box: bool=True):
        ...
        for l, line in enumerate(lines_split):
            ...
            if is_box:
                yolo_coordinates[l,:] = np.array((float(line_split[1]), float(line_split[2]), float(line_split[3]), float(line_split[4])))
                ...
                yolo_coordinates.append(coordinates.reshape((int(coordinates.size/2),2)))
    ...
    def get_text_file(self, file_name: str):
        return self.provider_annotations.get_bytes(self.get_full_path_annotation(file_name)).decode()