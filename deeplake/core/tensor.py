...
class Tensor:
    ...
    def extend(
        ...
        or a sequence of :func:`dl.read` outputs, which can be used to load files. See examples down below.
        ...
        Example:
            ...
            0
            >>> tensor.extend([
                    dl.read("path/to/image1"),
                    dl.read("path/to/image2"),
                ])
            >>> len(tensor)
            2
    ...
    def append(self, sample: InputSample):
        """Appends a single sample to the end of the tensor. Can be an array, scalar value, or the return value from :func:`dl.read`,
        ...
        Examples:
            ...
            >>> len(tensor)
            0
            >>> tensor.append(dl.read("path/to/file"))
            >>> len(tensor)
            1

        Args:
            sample (InputSample): The data to append to the tensor. :class:`~dl.core.sample.Sample` is generated by :func:`dl.read`. See the above examples.
        """
    ...
    def shape_interval(self) -> ShapeInterval:
        """Returns a :class:`~dl.util.shape_interval.ShapeInterval` object that describes this tensor's shape more accurately. Length is included.
    ...
    def __setitem__(self, item: Union[int, slice], value: Any):
        ...
        if (
            dl.constants._ENABLE_RANDOM_ASSIGNMENT
