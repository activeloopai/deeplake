...
class ComputeFunction:
    ...
    def eval(
        self,
        data_in,
        ds_out: Optional[dl.Dataset] = None,
        num_workers: int = 0,
...
class Pipeline:
    def __init__(self, functions: List[ComputeFunction]):
        """Takes a list of functions decorated using dl.compute and creates a pipeline that can be evaluated using .eval"""
    ...
    def eval(
        ...
        **kwargs,
        ...
        if isinstance(data_in, dl.Dataset):
...
def compose(functions: List[ComputeFunction]):  # noqa: DAR101, DAR102, DAR201, DAR401
    """Takes a list of functions decorated using :func:`dl.compute` and creates a pipeline that can be evaluated using .eval

    Example::

        pipeline = dl.compose([my_fn(a=3), another_function(b=2)])
...
def compute(
    ...
    Example::
        ...
        pipeline = dl.compose([my_fn(a, b), another_function(x=2)])
        pipeline.eval(data_in, ds_out, scheduler="processed", num_workers=2)
