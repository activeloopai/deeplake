...
def read_image(sample_in, samples_out):
    samples_out.image.append(dl.read(sample_in))
...
def add_link(sample_in, samples_out):
    samples_out.abc.append(dl.link(sample_in))


@dl.compute
def add_images(i, sample_out):
    for i in range(5):
        image = dl.read(get_dummy_data_path("images/flower.png"))
...
def retrieve_objects_from_memory(object_type=dl.core.sample.Sample):
...
def test_single_transform_hub_dataset(ds, scheduler):
    data_in = dl.dataset("./test/single_transform_hub_dataset", overwrite=True)
    with data_in:
...
def test_groups(local_ds):
    with CliRunner().isolated_filesystem():
        with dl.dataset("./test/transform_hub_in_generic") as data_in:
        ...
        data_in = dl.dataset("./test/transform_hub_in_generic")
        ds_out = local_ds
...
def test_single_transform_hub_dataset_htypes(local_ds, num_workers, scheduler):
    data_in = dl.dataset("./test/single_transform_hub_dataset_htypes", overwrite=True)
    with data_in:
...
def test_chain_transform_list_small(local_ds, scheduler):
    ...
    ds_out.create_tensor("image")
    ds_out.create_tensor("label")
    pipeline = dl.compose([fn1(mul=5, copy=2), fn2(mul=3, copy=3)])
    pipeline.eval(
        ls,
        ds_out,
        num_workers=TRANSFORM_TEST_NUM_WORKERS,
...
def test_chain_transform_list_big(local_ds, scheduler):
    ...
    ds_out.create_tensor("image")
    ds_out.create_tensor("label")
    pipeline = dl.compose([fn3(mul=5, copy=2), fn2(mul=3, copy=3)])
    pipeline.eval(
        ls,
        ds_out,
        num_workers=TRANSFORM_TEST_NUM_WORKERS,
...
def test_transform_hub_read_pipeline(local_ds, cat_path, sample_compression, scheduler):
    ...
    ds_out = local_ds
    ds_out.create_tensor("image", htype="image", sample_compression=sample_compression)
    pipeline = dl.compose([read_image(), crop_image(copy=2)])
    pipeline.eval(
        data_in,
        ds_out,
        num_workers=TRANSFORM_TEST_NUM_WORKERS,
    ...
    with CliRunner().isolated_filesystem():
        ...
        with data_in:
            ...
            for i in range(1, 100):
                data_in.image.append(i * np.ones((i, i), dtype="uint8"))
                data_in.label.append(i * np.ones((1,), dtype="uint32"))
        ds_out = dl.like("./transform_hub_like", data_in)
        fn2(copy=1, mul=2).eval(
            data_in,
            ds_out,
            num_workers=TRANSFORM_TEST_NUM_WORKERS,
...
def test_transform_pass_through():
    data_in = dl.dataset("mem://ds1")
    data_in.create_tensor("image", htype="image", sample_compression="png")
    data_in.create_tensor("label", htype="class_label")
    for i in range(1, 100):
        data_in.image.append(i * np.ones((i, i), dtype="uint8"))
        data_in.label.append(i * np.ones((1,), dtype="uint32"))
    ds_out = dl.dataset("mem://ds2")
...
def test_transform_pad_data_in(local_ds):
    ...
    ds2 = dl.dataset("./data/unequal2", overwrite=True)
    ds2.create_tensor("x")
    ds2.create_tensor("y")
