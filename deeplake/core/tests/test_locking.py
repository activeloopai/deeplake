...
class VM(object):
    ...
    def __enter__(self):
        ...
        self._locks = dl.core.lock._LOCKS.copy()
        dl.core.lock._LOCKS.clear()

    def __exit__(self, *args, **kwargs):
        uuid.getnode = self._getnode
        dl.core.lock._LOCKS.update(self._locks)
...
def test_dataset_locking(ds_generator):
    ...
    with VM():
        ...
        DATASET_LOCK_VALIDITY = dl.constants.DATASET_LOCK_VALIDITY
        # Temporarily set validity to 1 second so we dont have to wait too long.
        dl.constants.DATASET_LOCK_VALIDITY = 1
        ...
        try:
            ...
            dl.constants.DATASET_LOCK_VALIDITY = DATASET_LOCK_VALIDITY
...
def test_lock_thread_leaking(s3_ds_generator):
    locks = dl.core.lock._LOCKS
    refs = dl.core.lock._REFS
