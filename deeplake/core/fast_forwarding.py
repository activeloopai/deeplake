...
def _check_version(v):
    ...
    comparison = version_compare(v, dl.__version__)
    if comparison > 0:
        warnings.warn(
            f"Loading a dataset that was created or updated with a newer version of dl. This could lead to corruption or unexpected errors! Dataset version: {v}, current hub version: {dl.__version__}. It's recommended that you update to a version of hub >= {v}."
        )
