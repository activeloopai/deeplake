...
def append_info(sample, link_creds=None):
    meta = {}
    if isinstance(sample, dl.core.linked_sample.LinkedSample):
        sample = read_linked_sample(
            sample.path, sample.creds_key, link_creds, verify=False
        )
    if isinstance(sample, dl.core.sample.Sample):
        meta = sample.meta
        meta["modified"] = False
    return meta
...
def read_linked_sample(
    ...
    if sample_path.startswith(("gcs://", "gcp://", "gs://", "s3://")):
        ...
        return dl.read(sample_path, storage=storage, verify=verify)
    elif sample_path.startswith(("http://", "https://")):
        creds = link_creds.get_creds(sample_creds_key)
        return dl.read(sample_path, verify=verify, creds=creds)
    return dl.read(sample_path, verify=verify)
