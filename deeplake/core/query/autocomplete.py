...
def _parse(s: str, ds: dl.Dataset) -> List[dict]:
    """Tokenizes a query string and assigns a token type to each token.

    Args:
        s (str): The query string
        ds (dl.Dataset): The dataset against which the query is run
...
def _parse_no_fail(s: str, ds: dl.Dataset):
...
def _initial_suggestions(ds: dl.Dataset):
...
def autocomplete(s: str, ds: dl.Dataset) -> dict:
    """Returns autocomplete suggestions and tokenizer result for a given query string and dataset.

    Args:
        s (str): The query string
        ds (dl.Dataset): The hub dataset against which the query is to be run.
