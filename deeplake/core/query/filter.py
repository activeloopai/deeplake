...
def _filter_function_to_query_text(filter_function):
    if isinstance(filter_function, dl.core.query.DatasetQuery):
...
def filter_dataset(
    dataset: dl.Dataset,
    filter_function: Callable[[dl.Dataset], bool],
    num_workers: int = 0,
...
def _get_vds_thread(vds: dl.Dataset, queue: Queue, num_samples: int):
    """Creates a thread which writes to a vds in background.

    Args:
        vds: (dl.Dataset) The vds to write to.
