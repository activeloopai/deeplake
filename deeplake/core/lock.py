...
class PersistentLock(Lock):
    ...
    Example:
        From machine 1:
        s3 = dl.core.storage.S3Provider(S3_URL)
        lock = dl.core.lock.Lock(s3)  # Works

        From machine 2:
        s3 = dl.core.storage.S3Provider(S3_URL)
        lock = dl.core.lock.Lock(s3)  # Raises LockedException
            ...
            while True:
                try:
                    if (
                        ...
                        >= dl.constants.DATASET_LOCK_VALIDITY
                    ):
                    ...
                    pass
                time.sleep(dl.constants.DATASET_LOCK_UPDATE_INTERVAL)
        ...
        if lock_bytes is not None:
            ...
            if nodeid:
                if nodeid == uuid.getnode():
                    ...
                    pass
                elif time.time() - timestamp < dl.constants.DATASET_LOCK_VALIDITY:
                    raise LockedException()
