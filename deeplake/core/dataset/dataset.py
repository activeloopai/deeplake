...
class Dataset:
    ...
    def create_tensor(
        ...
        **kwargs,
        ...
        Examples:
            ...
            >>> ds.videos.append(dl.read("videos/sample_video.mp4"))
        ...
        Args:
            ...
            create_sample_info_tensor (bool): If ``True``, meta data of individual samples will be saved in a hidden tensor. This data can be accessed via :attr:`tensor[i].sample_info <dl.core.tensor.Tensor.sample_info>`.
    ...
    def delete_tensor(self, name: str, large_ok: bool = False):
        ...
        if not large_ok:
            ...
            if size_approx > dl.constants.DELETE_SAFETY_SIZE:
                logger.info(
    ...
    def filter(
        ...
        Args:
            function (Callable, str): Filter function that takes sample as argument and returns ``True`` / ``False``
                ...
                See :class:`dl.core.query.query.DatasetQuery` for more details.
    ...
    def rename(self, path: Union[str, pathlib.Path]):
        ...
        Example:
            ...
            >>> ds = dl.load("hub://username/dataset")
    ...
    def delete(self, large_ok=False):
        ...
        if not large_ok:
            ...
            if size > dl.constants.DELETE_SAFETY_SIZE:
                logger.info(
    ...
    def _save_view_in_user_queries_dataset(
        ...
        try:
            ...
            dl.delete(queries_ds_path, force=True)
            queries_ds = dl.empty(queries_ds_path, verbose=False)
        ...
        vds = dl.empty(path, overwrite=True, verbose=False)
        ...
        **ds_args,
        ...
        Args:
            ...
            path (Optional, str, pathlib.Path): - The VDS will be saved as a standalone dataset at the specified path.
                ...
                - If the user doesn't have write access to the source dataset and the source dataset is a hub cloud dataset, then the VDS is saved is saved under the user's hub account and can be accessed using ``dl.load(f"hub://{username}/queries/{query_hash}")``.
            ...
            verbose (bool): If ``True``, logs will be printed. Defaults to ``True``.
            ds_args (dict): Additional args for creating VDS when path is specified. (See documentation for :func:`dl.dataset()`)
        ...
        Note:
            ...
            :func:`Dataset.load_view`, :func:`Dataset.delete_view` methods. They have to be loaded using :func:`dl.load`.
        """
        ...
        **ds_args,
        ...
        Args:
            path (Optional, str, pathlib.Path): If specified, the VDS will saved as a standalone dataset at the specified path. If not,
                ...
                is saved under the user's hub account and can be accessed using dl.load(f"hub://{username}/queries/{query_hash}").
            ...
            _ret_ds (bool): If ``True``, the VDS is retured as such without converting it to a view. If ``False``, the VDS path is returned.
                Default False.
            ds_args (dict): Additional args for creating VDS when path is specified. (See documentation for `dl.dataset()`)
        ...
        if vds is None:
            if path is None:
                ...
                if self.read_only and not (self._view_base or self)._locked_out:
                    if isinstance(self, dl.core.dataset.HubCloudDataset):
    ...
    def _get_queries_ds_from_user_account():
        ...
        if username == "public":
            return
        try:
            return dl.load(f"hub://{username}/queries", verbose=False)
        except DatasetHandlerError:
            return
    ...
    def load_view(
        ...
        """Loads the view and returns the :class:`~dl.core.dataset.dataset.Dataset` by id. Equivalent to ds.get_view(id).load().
    ...
    def _sub_ds(
        ...
        if self.path.startswith("hub://"):
            ...
            cls = dl.core.dataset.HubCloudDataset
        else:
            path = sub_storage.root
            cls = dl.core.dataset.Dataset
    ...
    def _link_tensors(
        ...
        Args:
            ...
            append_f (str): Name of the linked tensor transform to be used for appending items to the destination tensor. This transform should be defined in `dl.core.tensor_link` module.
            update_f (str): Name of the linked tensor transform to be used for updating items in the destination tensor. This transform should be defined in `dl.core.tensor_link` module.
    ...
    def _copy(
        ...
        dest_ds = dl.api.dataset.dataset._like(
            dest,
            self,
            tensors=tensors,
            creds=creds,
            ...
            unlink=[
                ...
                if (
                    self.tensors[t].base_htype != "video" or dl.constants._UNLINK_VIDEOS
                )
            ]
            if unlink
            else False,
        ...
        try:
            for tensor in dest_ds.tensors:
                ...
                copy_f = (
                    ...
                    if unlink
                    and src.is_link
                    and (src.base_htype != "video" or dl.constants._UNLINK_VIDEOS)
                    else _copy_tensor
                )
                if progressbar:
                    sys.stderr.write(f"Copying tensor: {tensor}.\n")
                dl.compute(copy_f, name="tensor copy transform")(
                    tensor_name=tensor
                ).eval(
                    self,
                    dest_ds,
