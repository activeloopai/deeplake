...
def compress_array(array: np.ndarray, compression: Optional[str]) -> bytes:
    ...
    Raises:
        UnsupportedCompressionError: If `compression` is unsupported. See `dl.compressions`.
    ...
    if compression not in dl.compressions:
    ...
    if compr_type == BYTE_COMPRESSION:
        ...
        raise NotImplementedError(
            "In order to store audio data, you should use `dl.read(path_to_file)`. Compressing raw data is not yet supported."
        )
    elif compr_type == VIDEO_COMPRESSION:
        raise NotImplementedError(
            "In order to store video data, you should use `dl.read(path_to_file)`. Compressing raw data is not yet supported."
        )
    elif compr_type == POINT_CLOUD_COMPRESSION:
        raise NotImplementedError(
            "In order to store point cloud data, you should use `dl.read(path_to_file)`. Compressing raw data is not yet supported."
        )
    if compression == "apng":
    ...
    try:
        ...
        out.close = (  # type: ignore
            lambda: None
        )  # sgi save handler will try to close the stream (see https://gitdl.com/python-pillow/Pillow/pull/5645)
...
def _fast_decompress(buf):
    ...
    while True:
        status, err_code, buf = enc.encode(
            bufsize
        )  # See https://gitdl.com/python-pillow/Pillow/blob/master/src/encode.c#L144
        if err_code:
            break
    if err_code < 0:
        raise Exception()  # caught by verify_compressed_file()
