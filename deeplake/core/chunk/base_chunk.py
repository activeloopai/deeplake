    ...
    def serialize_sample(
        ...
        if isinstance(incoming_sample, LinkedSample):
            if self.tensor_meta.is_link:
                ...
                raise ValueError(
                    "dl.link() samples can only be appended to linked tensors. To create linked tensors, include link in htype during create_tensor, for example 'link[image]'."
                )
        ...
        elif isinstance(incoming_sample, dl.core.tensor.Tensor):
