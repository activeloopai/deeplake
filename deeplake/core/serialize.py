...
def get_header_from_url(url: str):
    ...
    enc_dtype = np.dtype(dl.constants.ENCODING_DTYPE)
...
def deserialize_sequence_or_creds_encoder(
    ...
    version = str(byts[1 : 1 + len_version], "ascii")
    enc = (
        np.frombuffer(byts[1 + len_version :], dtype=dl.constants.ENCODING_DTYPE)
        .reshape(-1, dim)
        .copy()
    )
    return version, enc
...
def text_to_bytes(sample, dtype, htype):
    if isinstance(sample, dl.core.tensor.Tensor):
        try:
...
def serialize_tensor(
    incoming_sample: "dl.core.tensor.Tensor",
    sample_compression: Optional[str],
    chunk_compression: Optional[str],
    dtype: str,
    htype: str,
