...
def store_data_slice_with_pbar(pg_callback, transform_input: Tuple) -> Dict:
    ...
    if isinstance(data_slice, dl.Dataset):
...
def add_cache_to_dataset_slice(
    ...
    dataset_slice = dl.core.dataset.dataset_factory(
...
def check_transform_ds_out(
    ds_out: dl.Dataset, scheduler: str, check_lengths: bool
) -> None:
...
def get_pbar_description(compute_functions: List):
    """Returns the description string for a dl.compute evaluation progress bar. Incoming list should be a list of `ComputeFunction`s."""
