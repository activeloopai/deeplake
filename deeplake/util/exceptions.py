...
class PathNotEmptyException(Exception):
    def __init__(self, use_hub=True):
        if use_hub:
            super().__init__(
                f"Please use a url that points to an existing Hub Dataset or an empty folder. If you wish to delete the folder and its contents, you may run dl.delete(dataset_path, force=True)."
            )
        else:
            super().__init__(
                f"Specified path is not empty. If you wish to delete the folder and its contents, you may run dl.delete(path, force=True)."
            )
...
class UnsupportedCompressionError(CompressionError):
    def __init__(self, compression: Optional[str], htype: Optional[str] = None):
        if htype:
            ...
            super().__init__(
                f"Compression '{compression}' is not supported. Supported compressions: {dl.compressions}."
            )
...
class SampleDecompressionError(CompressionError):
    def __init__(self):
        super().__init__(
            f"Could not decompress sample buffer into an array. Either the sample's buffer is corrupted, or it is in an unsupported format. Supported compressions: {dl.compressions}."
        )
...
class TensorMetaMissingRequiredValue(MetaError):
    def __init__(self, htype: str, key: str):
        ...
        if key == "sample_compression":
            extra = f"`sample_compression` may be `None` if you want your '{htype}' data to be uncompressed. Available compressors: {dl.compressions}"
...
class HubComposeEmptyListError(TransformError):
    def __init__(self, message="Cannot dl.compose an empty list."):
        super().__init__(message)
...
class HubComposeIncompatibleFunction(TransformError):
    def __init__(self, index: int):
        super().__init__(
            f"The element passed to dl.compose at index {index} is incompatible. Ensure that functions are all decorated with dl.compute decorator and instead of passing my_fn, use my_fn() or my_fn(arg1=5, arg2=3) in the list."
        )
