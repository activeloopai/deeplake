...
def aws_cli_copy(samples=30, chunksize=None, name="aws cli"):
    ...
    """
    path = "/tmp/create_s3"
    ds = gen_dataset([(samples, 256, 256), (samples, 256, 256)], chunksize=chunksize)
    t0 = time.time()
    ds = ds.store(path)
    t1 = time.time()
    ...
    t3 = time.time()
    return {"name": name, "upload": t2-t1, "download": t3-t2, "write_to_fs": t1-t0}
    
def upload_and_download(samples=30, chunksize=None, name="hub"):
    """
    Uploads dataset into S3 and then downlods using hub package
    """
    ds = gen_dataset([(samples, 256, 256), (samples, 256, 256)], chunksize=1)
    t1 = time.time()
