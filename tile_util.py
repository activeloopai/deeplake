...
def validate_not_serialized(tiles: np.ndarray, operation: str):
    if type(tiles.flatten()[0]) == bytes:
        raise TypeError(f"Before {operation}, you should deserialize them.")
...
def get_tile_shapes(tiles: np.ndarray) -> np.ndarray:
    # TODO: docstring (explain what tile shapes is and why it's different than the output of the tile shape optimizer)
